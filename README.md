# Alok Kumar

B.Tech Computer Science · IEM Kolkata ('27)

[LinkedIn](https://linkedin.com/in/alok-devforge) · [GitHub](https://github.com/alok-devforge) · [alok.kumar2023@iem.edu.in](mailto:alok.kumar2023@iem.edu.in)

![Profile Views](https://komarev.com/ghpvc/?username=alok-devforge&style=flat-square&color=grey) [![LeetCode](https://img.shields.io/badge/LeetCode-alok__devforge-FFA116?style=flat-square&logo=leetcode&logoColor=black)](https://leetcode.com/u/alok_devforge/)

---

Pre-final year CS student focused on DSA (C++), full-stack development, and applied ML. I build end-to-end systems — from real-time backends to AI-powered tools — with a strong record in competitive hackathons.

---

## Education

**B.Tech CST** · IEM Kolkata · 2023–2027 · CGPA 8.81  
**Class XII** · New Horizon School · 2022  
**Class X** · St. Joseph School · 2020

---

## Skills

**Languages:** C++, Python, Java, C  
**ML / Vision:** TensorFlow, YOLOv8, OpenCV, Scikit-learn, Grad-CAM  
**Backend:** Node.js, Express, FastAPI, Socket.IO  
**Tools:** Git, HTML/CSS

---

## Projects

**[MediScan](https://github.com/alok-devforge/MediScan)** · Python · FastAPI · YOLOv8 · Grad-CAM  
Bone fracture detection system with 99.8% accuracy and Explainable AI visualizations.

**[MAITRI](https://github.com/TechFreak2003/MAITRI)** · Node.js · Socket.IO · Geo-fencing  
Real-time human-wildlife conflict tracker with dynamic safe-path routing and proximity alerts. *(StatusCode2 Hackathon)*

**Rail Sathi** · Node.js · Socket.IO  
Railway companion app with crowdsourced coach density, network beacon boarding assist, and AI search. *(Diversion 2K26)*

**Diabetic Retinopathy Detection** · ResNet · EfficientNet · ViT  
Research project — stacking ensemble model with Grad-CAM interpretability. *(Guide: Prof. Dr. Subhalaxmi Chakraborty)*

---

## Recognition

- Finalist · IIT Madras Shaastra Coding Contest 2025
- 1st Runner-Up · Technologia 1.0 Hackathon 2025
- Finalist · StatusCode2 2025 — Top 100 / 1,400+ teams (IISER Kolkata)
- Zonal Finalist · Anti-Piracy Challenge 2025 (WAVES, Govt. of India)
- 1st Place · CreaXion Project Competition 2025

---

## Certifications

Applied Cryptography Specialization · Coursera (2025)  
C++ Programming Professional Certificate · OpenEDG / C++ Institute (2025)

